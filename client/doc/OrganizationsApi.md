...
> List<ProductScheme> getOrganizationProducts(organizationId)
...
try {
    final response = api.getOrganizationProducts(organizationId);
    print(response);
} catch on DioException (e) {
    print('Exception when calling OrganizationsApi->getOrganizationProducts: $e\n');
}
```
